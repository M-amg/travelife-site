<form action="{{route('front.packages')}}" autocomplete="off" method="get">
    <div class="container py-3 booking-search">
        <div class="col-lg-12 pb-3">Plus de 1 500 offres voyages vers 70 destinations sélectionnées pour vous au meilleur prix garanti !</div>
        <div class="row">
            ...
            <div class="form-group col-md col-12 px-2 pr-0">
                <select id="Period" class="form-control-new newsearch" name="period">
                    ...
                    @foreach($periods as $period)
                        <option value="{{$period}}">
                            @lang('generate.'.substr($period, 0, -5))-{{ substr($period, -4, 5) }} 
                        </option>
                    @endforeach
                </select>
            </div>
